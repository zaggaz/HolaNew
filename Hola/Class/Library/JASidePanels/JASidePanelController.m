...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver: self
                                             selector: @selector(onTouchLeftBtn:)
                                                 name: HOME_LEFTBTN_TOUCH
                                               object: nil];
    
    [[NSNotificationCenter defaultCenter] addObserver: self
                                             selector: @selector(onTouchRightBtn:)
                                                 name: HOME_RIGHTBTN_TOUCH
                                               object: nil];
    ...
    switch (self.state) {
        case JASidePanelCenterVisible: {
            if (deltaX > 0.0f) {
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName: PANEL_LEFT_WILL_SHOW object: nil];
            } else {
                [self _showRightPanel:YES bounce:self.bounceOnSidePanelOpen];
                
                [[NSNotificationCenter defaultCenter] postNotificationName: PANEL_RIGHT_WILL_SHOW object: nil];
            }
            break;
		}
        case JASidePanelLeftVisible: {
...
- (void)_showCenterPanel:(BOOL)animated bounce:(BOOL)shouldBounce {
    ...
    [self _adjustCenterFrame];
    [[NSNotificationCenter defaultCenter] postNotificationName: @"UPDATE_MAP_PIN" object: nil];
...
- (void)onTouchLeftBtn: (NSNotification *)notification {
    [self toggleLeftPanel: nil];
}

- (void)onTouchRightBtn: (NSNotification *)notification {
